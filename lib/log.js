...
export function createLogger({ emailFull = false, logFile = null } = {}) {
  ...
  function sanitizeValue(key, value) {
    ...
    if (Array.isArray(value)) {
      ...
      // Matches both input modes: `content` (base64) and `path` (read from disk).
      // Only the basename of a `path` is logged, so logs stay filenames-only and
      // don't record the user's directory layout.
      if (value[0] && typeof value[0] === 'object' && ('content' in value[0] || 'path' in value[0])) {
        return {
          _count: value.length,
          names: value.slice(0, 20).map(a => a.name || (a.path ? basename(a.path) : undefined)),
        };
      }
