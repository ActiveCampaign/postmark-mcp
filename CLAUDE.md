...
- **Smoke test (mutating):** `node smoke-test-mutating.mjs` — runs full create→edit→delete lifecycles for templates (including layout binding), webhooks, and suppressions, plus real email sends between two configured verified addresses, including one with a valid inline-base64 attachment, one with an attachment read from disk via `path`, one with a deliberately corrupted attachment, and one with a nonexistent `path` (the last two expected to be rejected before anything sends). Cleans up after itself. Created from [smoke-test-mutating.example.mjs](smoke-test-mutating.example.mjs); edit `SENDER` and `RECIPIENT` before running. The script refuses to run with the placeholder values still in place.
...
All four sending tools (`sendEmail`, `sendEmailWithTemplate`, and per-message/per-recipient in `sendBatch` / `sendBatchWithTemplate`) accept an optional `attachments` array (max 10 per message, shared `attachmentsSchema` from `lib/attachments.js`) mapped to Postmark's native `Attachments` field.

Each attachment supplies its file by **either `path` or `content`** (enforced by a `superRefine` on `attachmentSchema`). **`path` is the preferred mode and the one to steer callers toward** — `resolveAttachmentPath` + `readFileSync` inside `validateAttachment` read the bytes server-side, inferring `name` from the basename and `contentType` from `EXTENSION_MIME_TYPES`. Because all four tools funnel through `validateAttachment`, path support is inherited everywhere; don't add a parallel path-reading code path in a tool handler.

`path` exists because **MCP has no way for a client to hand a server the bytes of a user-uploaded file** — tool arguments are JSON only, image/blob content blocks are result-side, roots carry `file://` URIs but never bytes (and Claude Desktop doesn't support roots). Anthropic's [File Uploads working group](https://modelcontextprotocol.io/community/working-groups/file-uploads) names this gap; [SEP-2631](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2631) is still draft. So the only alternative is a model emitting base64 into the tool call, which for a 70 KB image is ~93k characters (~25-30k output tokens) and reliably **truncates** — the observed failure was `length is not a multiple of 4` on three attempts plus one where `contentType` (the field right after `content` in schema order) went missing entirely, the signature of a call that ran past its output limit. It also **fabricates** when the model only ever saw the file as a rendered image. Every base64-path error message therefore ends by directing the caller to `path` and explicitly saying not to retry with base64 — **keep that redirect if you touch these messages**, since a plain "invalid base64" reads as "fix your encoding" and produces an infinite retry loop. If a file was attached to the conversation rather than saved to disk, it genuinely cannot be sent; the error says to ask the user to save it.

**`path` is resolved on the machine running the server, which is not necessarily where the caller lives.** This is a local stdio server, so it reads the host that spawned it; an assistant calling it may be reasoning inside its own container where conversation uploads are mounted under `/mnt/user-data/` and similar. Those paths are real for the caller and unreachable here. `SANDBOX_PATH_PREFIXES` exists purely to detect that case and return "different filesystem" instead of a bare "file not found" — it never allows or denies a read, so don't repurpose it as a security control. Confirmed topology for the observed failure: Claude Desktop's config runs `node /path/to/index.js` as a local subprocess on the user's Mac, while the uploaded file lived only in the model's container. A file that exists **only** as a conversation upload therefore cannot be attached by either mode; a file on the user's machine works fine.

`POSTMARK_ATTACHMENT_DIR` (optional) confines `path` reads to one directory, resolving both sides through `realpathSync` so a symlink inside it can't escape. Unset means any readable file — reading a local file and emailing it outbound is an exfiltration path when a filename comes from untrusted content, so don't remove this guard.

`lib/log.js`'s `sanitizeArgs` detects attachment arrays by `content` **or** `path` and logs `basename(path)` only — a `path`-only attachment would otherwise fall through to the generic-array branch and lose its filename from the audit log. Keep both keys in that check if the schema gains another input mode.

`validateAttachment` rejects, in order: a forbidden file extension (`FORBIDDEN_ATTACHMENT_EXTENSIONS` — Postmark's documented list, matched case-insensitively against the filename only), malformed base64 (charset/padding/round-trip re-encode — `Buffer.from()` silently drops invalid runs rather than throwing, so the round-trip comparison is what actually catches that; **skipped in `path` mode**, where the base64 is generated server-side and well-formed by construction), a single attachment already over Postmark's 10 MB total-message ceiling, and (when `contentType` matches a known entry in `ATTACHMENT_STRUCTURE_VALIDATORS` — PNG/JPEG/GIF/WEBP/PDF) a failed structural-integrity check. That last check is deliberately more than a magic-bytes comparison — **do not regress it back to a header-only check**. PNG walks the full chunk stream and verifies every chunk's CRC32; JPEG/GIF require both the start marker *and* the end-of-file marker; WEBP's declared RIFF size must equal the actual buffer length; PDF must end in `%%EOF`. This exists because a header-only check was tried first and missed a real corrupted attachment in production testing: the header (first ~8 bytes) survived a model's base64 transcription intact while the body didn't, so the file still "matched the signature" while being unrenderable — same filename, wrong size, no image. `assertMessageSize` additionally enforces Postmark's 5 MB per-body-section / 10 MB per-message-total limits (body sizes are skipped for template-based sends, since Postmark renders those server-side and the size isn't known client-side), and `assertBatchPayloadSize` enforces the 50 MB total-batch-payload limit by summing across all messages/recipients in one `sendBatch`/`sendBatchWithTemplate` call. All these numbers are sourced from Postmark's docs (linked in `lib/attachments.js`) — re-verify against the live docs before changing any of them, don't just adjust to make a test pass.

Structural validation still matters in `path` mode (the file on disk can itself be corrupt) but is load-bearing in `content` mode, which remains supported for data that genuinely arrives verbatim. The fallback it guards against is hand-embedding base64 in `htmlBody`, which has no validation at all and silently corrupts. When adding attachment support to a new tool, reuse `attachmentsSchema` / `buildAttachments` (single message) or call `validateAttachment` directly per item (batch, so each error can be labeled with its index) — and call `assertMessageSize`/`assertBatchPayloadSize` too, not just `validateAttachment`, or the new tool will silently skip the size limits.
...
Optional: `AGENT_LABEL`, `WEBHOOK_URL_ALLOWLIST`, `POSTMARK_ATTACHMENT_DIR` (confines attachment `path` reads to one directory), `LOG_FILE`, `LOG_EMAIL_FULL` — all documented in [.env.example](.env.example) and the README's Optional env var table.
